...
public class Expression {
     ...
     * 带检错的表达式生成器（一次生成一条）
     *
     * @param range 限定表达式中所有数值的最大值
     * @return 返回生成的表达式
     */
    public static String createExpressionWithCheck(int range) {
        String exp;
        for (; ; ) {
            exp = createExpression(range);
            if (!hasError(exp)) {
                return exp;
            }
        }
